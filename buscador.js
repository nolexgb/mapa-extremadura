...
window.addEventListener('load', async () => {
  ...
  async function cargarDatos() {
    ...
    counts['CONGDEX'] = entidadesCongdex.length;
    ...
    categorias.forEach(cat => {
      ...
      label.style.backgroundColor = colorMap[cat] || '#999';
    ...
    function renderSuggestions(q) {
      ...
      const nq = norm(q);
      const results = todasEntidades.filter(f => norm(f.properties?.nombre_entidad || '').includes(nq)).slice(0, 15);
    ...
    box.addEventListener('click', (e) => {
      ...
      const pool = todasEntidades.filter(f => norm(f.properties.nombre_entidad || '').includes(q)).slice(0, 15);
