...
window.addEventListener('load', async () => {
  ...
  // === Cargar GeoJSON principal + CONGDEX ===
  const cargarDatos = async () => {
    const resEnt = await fetch('entidades.geojson', { cache: 'no-store' });
    const geojsonEnt = await resEnt.json();

    const resCong = await fetch('congdex.geojson', { cache: 'no-store' });
    ...
    categorias.push('CONGDEX'); // Nueva categoría
