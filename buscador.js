...
map.on('load', () => {
  fetch('entidades.geojson')
    .then(response => {
      if (!response.ok) throw new Error("No se pudo cargar entidades.geojson");
    ...
    .catch(err => {
      ...
      alert("Error: no se pudo cargar el mapa. Verifica el archivo entidades.geojson y los permisos del repositorio.");
    });
});
